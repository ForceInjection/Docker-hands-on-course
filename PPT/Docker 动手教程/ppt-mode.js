...
class PPTController {
    ...
    // 检测当前页面编号
    detectCurrentPage() {
        console.log('=== 检测当前页面编号 ===');
        const currentPath = window.location.pathname;
        console.log('当前路径:', currentPath);
        
        // 尝试多种匹配模式
        let match = currentPath.match(/page_(\d+)\.html/);
        if (!match) {
            // 如果路径包含编码字符，尝试解码
            const decodedPath = decodeURIComponent(currentPath);
            console.log('解码后路径:', decodedPath);
            match = decodedPath.match(/page_(\d+)\.html/);
        }
        
        if (match) {
            const pageNum = parseInt(match[1]);
            console.log('检测到页面编号:', pageNum);
            this.currentPage = pageNum;
        } else {
            console.log('未能检测到页面编号，使用默认值 1');
            this.currentPage = 1;
        }
        
        console.log('最终页面编号:', this.currentPage);
    ...
    bindEvents() {
        ...
        const fullscreenBtn = document.getElementById('ppt-fullscreen');
        const overviewBtn = document.getElementById('ppt-overview');
        
        if (toggleBtn) {
            toggleBtn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log('PPT切换按钮被点击');
                this.togglePPTMode();
            });
        }
    ...
    updatePageIndicator() {
        ...
        if (indicator) {
            const newText = `${this.currentPage} / ${this.totalPages}`;
            console.log('更新指示器文本为:', newText);
            indicator.textContent = newText;
        } else {
            console.log('未找到页面指示器元素');
        }
    }
}
